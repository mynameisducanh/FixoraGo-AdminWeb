...
const getTotalUserRegister = async () => {
  const res = await userApi.getTotalUser();
...
onMounted(async () => {
  await getTotalUserRegister();
  dataCard.value = {
    totalRevenue: 45231.89,
    totalRevenueDesc: 20.1 / 100,
    subscriptions: totalUser.value,
...
<template>
  <div class="w-full flex flex-col gap-4">
    <div class="flex flex-wrap items-center justify-between gap-2">
      <h2 class="text-2xl font-bold tracking-tight">Dashboard {{ config.public.apiBase }}</h2>
      ...
      <div class="grid gap-4 lg:grid-cols-4 md:grid-cols-2 md:gap-8">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p class="text-xs text-muted-foreground">
              <NumberFlow :value="dataCard.activeNowDesc" prefix="+" />
              since last hour
            </p>
          </CardContent>
        </Card>
